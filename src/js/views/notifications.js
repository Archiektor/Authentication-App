...
function notifyContainerTemplate() {
    return `<div class = 'notify-container' style='position: fixed; top: 10px; right: 10px; z-index: 11'></div>`;
}
...
export function notify({msg = 'Info message', className = 'alert-info', timeout = 2000} = {}) {
    ...
    const template = alertTemplate(msg, className);
    const container = getContainer();

    container.insertAdjacentHTML('beforeend', template, index);

    setTimeout(() => deleteNotify(index), timeout)
}

export function deleteNotify(index) {
    let alert;
